...
import { faAddressCard, faCalendarAlt, faCircleHalfStroke, faGear, faPerson, faTachometerAlt } from '@fortawesome/free-solid-svg-icons';
    ...
    <div className={`fixed top-0 left-0 bg-black w-24 h-24 rounded-br-full transition-all duration-1000 ease-in-out z-10 ${isOpen ? 'w-72 h-72' : ''}`}>
      ...
      {isOpen && (
        <div className="absolute top-0 left-0 z-10 flex flex-col space-y-4 p-4 h-full w-full">
          <button><FontAwesomeIcon icon={faCircleHalfStroke} className="text-white absolute transform rotate-45 top-[10%] right-[10%] text-6xl" /></button>
          <button><FontAwesomeIcon icon={faAddressCard} className="text-white absolute top-[40%] left-[40%] text-6xl" /></button>
          <button><FontAwesomeIcon icon={faGear} className="text-white absolute transform -rotate-45 top-[65%] left-[5%] text-6xl" /></button>
        </div>
      )}
    </div>
  );
